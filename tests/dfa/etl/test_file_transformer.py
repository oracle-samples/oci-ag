...
class TestFileTransformer(unittest.TestCase):
    ...
    def test_access_bundle(self):
        ...
        self.assertEqual(self.transformer._tenancy_id, 'ocid1.tenancy.oc1..aaaaaaaazp2vvzjsn6newkqrpkwndxpdoixtqfgyhnf4y24h7d5ny2639054')
        self.assertEqual(self.transformer._service_instance_id, 'ocid1.agcsgovernanceinstance.oc1.iad.amaaaaaaebkbezqaadpvwolr4raumlz3uxdgczwbqkalpcoo7qcu2r639054')
    ...
    def test_policy_to_resource_mapping(self):
        ...
        with self.assertLogs('dfa.adw.query_builders.base_query_builder', level='INFO') as logs:
            self.transformer.load_data()
            self.assertTrue(self.check_logs(logs.output, '9997 policy statement resource mapping events'))
