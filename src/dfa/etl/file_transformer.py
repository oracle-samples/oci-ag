...
class FileTransformer(AbstractTransformer):
    ...
    def _set_raw_event_data(self, event_data):
        ...
        if "jsonl" in self._object_name:
            ...
            raw_data = json.loads(json.loads(content)["data"])
            if isinstance(raw_data, dict):
                self.logger.info("we have a dict")
                self._raw_events = [raw_data]
            elif isinstance(raw_data, list):
                self._raw_events = raw_data
            else:
                self.logger.info("Cannot process event content - unknown format was found.")
            if "headers" in json.loads(content):
                if "messageType" in json.loads(content)["headers"]:
                    self._event_object_type = json.loads(content)["headers"]["messageType"]
                if "operation" in json.loads(content)["headers"]:
                    self._operation_type = json.loads(content)["headers"]["operation"]
                if "eventTime" in json.loads(content)["headers"]:
                    self._event_timestamp = json.loads(content)["headers"]["eventTime"]
                if "tenancyId" in json.loads(content)["headers"]:
                    self._tenancy_id = json.loads(content)["headers"]["tenancyId"]
                if "serviceInstanceId" in json.loads(content)["headers"]:
                    self._service_instance_id = json.loads(content)["headers"]["serviceInstanceId"]
    ...
    def chunk_prepared_events(self, chunk_size=10000):
        self.logger.info("Splitting %d events into chunks", len(self._prepared_events))
        chunks = []
