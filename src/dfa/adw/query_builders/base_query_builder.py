...
class InsertManyQueryBuilder:
    def get_operation_sql(self, query_builder, events, date_columns):
        ...
        for event_group_column_name in event.keys():
            ...
            insert_column_list.append(event_group_column_name.upper())
            insert_data.append(event[event_group_column_name])
            insert_parameters.append(Parameter(f":{event_group_column_name.lower()}"))
    ...
    def get_input_sizes(self, events):
        ...
        for column_name in events_df.columns:
            if events_df[column_name].dtype != "object" or events_df[column_name].dtype != "str":
                input_sizes[column_name] = None
                continue
            input_sizes[column_name] = int(events_df[column_name].str.len().max().item())
