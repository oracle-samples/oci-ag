...
class RoleStateUpdateQueryBuilder(RoleStateQueryBuilder):
    def executemany_sql_for_events(self):
        self.logger.info(
            "Using bulk insert / update operations for %d role events", len(self.events)
        )

        if len(self.events) == 0:
            self.logger.info("No events to process by role query builder")
            return
        ...
        input_sizes = InsertManyQueryBuilder().get_input_sizes(
            RoleStateTable().get_column_list_definition_for_table_ddl()
        )
        ...
        self.logger.info(
            "Removing outdated rows for %d unique role id, timestamp pairs",
