...
class PolicyStatementResourceMappingStateUpdateQueryBuilder(
    ...
    def executemany_sql_for_events(self):
        self.logger.info(
            "Using bulk insert / update operations for %d policy statement resource mapping events",
            len(self.events),
        )

        if len(self.events) == 0:
            self.logger.info("No events to process by policy stmt resource mapping query builder")
            return
    ...
    def execute_sql_for_events(self):
        ...
        for event in self.events:
            delete_sql = DeleteQueryBuilder().get_operation_sql(
                self, event, ["policy_statement_id", "resource_id", "service_instance_id", "tenancy_id"]
            )
            ...
            if counter % self.commit_interval == 0:
                self.logger.info("Committing work on interval")
                AdwConnection.commit()
