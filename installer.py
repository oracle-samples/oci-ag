...
def setup(application_ocid):
    ...
    if os.path.exists(wallet_extract_location) and os.path.isdir(wallet_extract_location):
        try:
            shutil.rmtree(wallet_extract_location)
...
def main():
    ...
    event_class.create_rule(
        f2s_function_id,
        os.environ["DFA_BUCKET_NAME"],
        os.environ["RESOURCE_NAME_PREFIX"] + "_file_to_state",
    )
    ...
    if os.environ.get("CREATE_TIME_SERIES", "false").lower() == "true":
        ...
        event_class.create_rule(
            f2ts_function_id,
            os.environ["DFA_BUCKET_NAME"],
            os.environ["RESOURCE_NAME_PREFIX"] + "_file_to_ts",
        )
